/**
 * commands/net.js — vellastra net 指令
 * =================================
 * 作为 vellastra 的子命令组，提供一站式 GitHub 网络检测、诊断与优化。
 *
 * 用法:
 *   vellastra net test               — 快速连通性检测
 *   vellastra net diagnose           — 全链路端点诊断（含延迟排序）
 *   vellastra net config             — 查看/应用 Git 网络优化配置
 *   vellastra net proxy              — 查看/设置 Git 代理
 *   vellastra net retry              — 测试重试机制
 */
...
export async function netTest(options) {
  ...
  if (result.ok) {
    ...
    console.log('   2. 运行 vellastra net diagnose 查看完整端点诊断');
    console.log(`   3. ${chalk.white('vellastra net config --apply')}  → 一键优化 Git 参数 ${chalk.green('(无需代理，推荐先试)')}`);
    console.log(`   4. ${chalk.white('vellastra net proxy --apply')}  → 自动检测系统代理并配置`);
  }
...
export async function netDiagnose() {
  ...
  if (best) {
    ...
    console.log(`   2. ${chalk.white('vellastra net config --apply')}  → 一键优化 Git 参数 ${chalk.green('(无需代理，推荐先试)')}`);
    console.log(`   3. ${chalk.white('vellastra net proxy --apply')}  → 自动检测系统代理并配置`);
  ...
  if (proxy.http || proxy.https) {
    ...
    console.log(`    ${chalk.yellow('💡 可通过')} ${chalk.white('vellastra net proxy --apply')} 将代理应用到 Git`);
  }
...
export async function netConfig(options) {
  if (options.apply) {
    ...
    console.log(`  ${chalk.dim('执行')} ${chalk.white('vellastra net config --apply')} ${chalk.dim('一键应用')}`);
    console.log('');
  }
}
...
function showProxyStatus() {
  ...
  console.log(`  ${chalk.dim('设置:')}   ${chalk.white('vellastra net proxy --apply http://127.0.0.1:7890')}`);
  console.log(`  ${chalk.dim('清除:')}   ${chalk.white('vellastra net proxy --clear')}`);
  console.log('');
}
...
async function applyProxy(proxyUrl) {
  if (!proxyUrl || proxyUrl === true) {
    ...
    if (!proxyUrl) {
      ...
      console.log(`  ${chalk.dim('用法:')} ${chalk.white('vellastra net proxy --apply http://127.0.0.1:7890')}`);
      return;
    }
  }
  ...
  if (allOk) {
    ...
    console.log(`  ${chalk.dim('可通过')} ${chalk.white('vellastra net proxy --clear')} ${chalk.dim('清除')}`);
  } else {
