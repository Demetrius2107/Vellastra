...
const pkg = { version: '1.0.0', name: 'blog' };
...
${chalk.cyan('║')}  ${chalk.yellow('名称')}: ${chalk.white('blog')}                                      ${chalk.cyan('║')}
...
${chalk.cyan('💡 常用操作速查:')}
  ${chalk.white('  blog init')}              首次使用，配置后端地址和 Token
  ${chalk.white('  blog push ./doc/*.md')}   推送本地文章
  ${chalk.white('  blog publish <id>')}      将草稿发布上线
  ${chalk.white('  blog pull <id>')}         拉取远程文章到本地
  ${chalk.white('  blog net test')}          检测 GitHub 连通性
  ${chalk.white('  blog net config --apply')} ${chalk.dim('← 一键优化 Git 网络参数（推荐）')}
  ${chalk.white('  blog net proxy --apply')}  ${chalk.dim('← 如果走代理/VPN 才需要')}
${chalk.cyan('────────────────────────────────────────────')}
`);
